...
troot_folder = Path("Trackman CSV Files")
troot_folder.mkdir(parents=True, exist_ok=True) 

tcsv_game_files = [file for file in troot_folder.glob("*.csv") if not (file.name.startswith("merged") or file.name.startswith("pitch_class"))]
if not tcsv_game_files:
    raise FileNotFoundError("No CSV files found in the 'Trackman CSV Files/' directory.")
