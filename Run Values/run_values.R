...
# Cleaning (the loaded function used an old version of the pbp dataset)

pbp_data_frame <- baseballr::load_ncaa_baseball_pbp(2023)

pbp  <- pbp_data_frame %>%
  mutate(
    away_text = description,
    home_text = description
  ) %>% 
  mutate(
    away_team = if_else(inning_top_bot == "top", batting, fielding),
    home_team = if_else(inning_top_bot == "bottom", batting, fielding)
  )

pbp <- pbp %>%
  tidyr::separate(score, into = c("away_score_before", "home_score_before"), sep = "-", convert = TRUE)

## Loaded in df parser function from https://github.com/davmiller/NCAA-baseball/blob/master/R/NCAA_base_pbp_parser.R ##
...
ncaa_parse=function(pbp_data_frame){
  ...
  pbp_data_frame=pbp_data_frame%>%
    ...
    select(year,game_date,game_pbp_id,away_team,home_team,inning,top_inning,away_score,home_score,away_text,home_text,bat_order,bat_name,r1_name,r2_name,r3_name,sub_fl,sub_in,sub_out,
